...
public interface ClassRepository extends CrudRepository<Clase, Long> {
	 ...
	 */
	@Query("SELECT new com.wedevol.iclass.core.view.response.ClassResponse(cla.id AS classId, cla.startTime, cla.endTime, cla.weekDay, cla.classDate, cla.status AS classStatus, cou.id AS courseId, cou.name AS courseName, 'instructor', ins.id AS userId, ins.firstName, ins.lastName, ins.phone, enr.price, enr.currency, cla.ratingToInstructor, cla.ratingToStudent) FROM Clase cla INNER JOIN Course cou ON cou.id = cla.courseId INNER JOIN Instructor ins ON ins.id = cla.instructorId INNER JOIN InstructorEnrollment enr ON enr.id.courseId = cou.id AND enr.id.instructorId = cla.instructorId AND enr.id.courseId = cla.courseId WHERE cla.studentId = :studentId AND cla.status in :classStatusList AND cla.ratingToStudent IS NULL AND (:actualDateStr < DATE_FORMAT(cla.classDate, '%Y%m%d') OR (:actualDateStr = DATE_FORMAT(cla.classDate, '%Y%m%d') AND :actualTime <= cla.startTime)) ORDER by cla.classDate ASC, cla.startTime ASC")
	 ...
	 * @return list of classes
	 */
	@Query("SELECT new com.wedevol.iclass.core.view.response.ClassResponse(cla.id AS classId, cla.startTime, cla.endTime, cla.weekDay, cla.classDate, cla.status AS classStatus, cou.id AS courseId, cou.name AS courseName, 'instructor', ins.id AS userId, ins.firstName, ins.lastName, ins.phone, enr.price, enr.currency, cla.ratingToInstructor, cla.ratingToStudent) FROM Clase cla INNER JOIN Course cou ON cou.id = cla.courseId INNER JOIN Instructor ins ON ins.id = cla.instructorId INNER JOIN InstructorEnrollment enr ON enr.id.courseId = cou.id AND enr.id.instructorId = cla.instructorId AND enr.id.courseId = cla.courseId WHERE cla.studentId = :studentId AND cla.status in :classStatusList ORDER by cla.classDate ASC, cla.startTime ASC")
