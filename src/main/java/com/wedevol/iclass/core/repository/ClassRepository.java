...
public interface ClassRepository extends CrudRepository<Clase, Long> {
	 ...
	 */
	@Query("SELECT new com.wedevol.iclass.core.entity.ClassFullInfo(cla.id AS classId, cla.startTime, cla.endTime, cla.weekDay, cla.classDate, cla.status AS classStatus, cou.id AS courseId, cou.name AS courseName, 'instructor', ins.id AS userId, ins.firstName, ins.lastName, ins.phone, enr.price, enr.currency, cla.ratingToInstructor, cla.ratingToStudent) FROM Clase cla, Course cou, Instructor ins, InstructorEnrollment enr WHERE ins.id = cla.instructorId AND cou.id = cla.courseId AND enr.id.courseId = cou.id AND enr.id.instructorId = cla.instructorId AND enr.id.courseId = cla.courseId AND cla.studentId = :studentId AND cla.status in :classStatusList AND (:actualDateStr < DATE_FORMAT(cla.classDate, '%Y%m%d') OR (:actualDateStr = DATE_FORMAT(cla.classDate, '%Y%m%d') AND :actualTime <= cla.startTime)) ORDER by cla.classDate ASC, cla.startTime ASC")
	...
	public List<ClassFullInfo> findHistoricClassesWithStudentIdWithClassStatusFilter(@Param("studentId") Long studentId,
	...
	public List<ClassFullInfo> findHistoricClassesWithInstructorIdWithClassStatusFilter(
